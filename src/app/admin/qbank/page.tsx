...
import { fetchNbmes } from "./actions";
...
export default function FoundationalReviewPage() {
  ...
  const [questions, setQuestions] = useState<NBMEQuestion[]>([]);

  useEffect(() => {
    const handler = setTimeout(async () => {
      const questions = await fetchNbmes(
...
function QuestionRow({ question }: { question: NBMEQuestion }) {
  if (question.step === "Step 1") return <StepOneRow question={question} />;
  return <StepTwoRow question={question} />;
}

function StepOneRow({ question }: { question: StepOneNBMEQuestion }) {
  ...
  return (
    <li className="space-y-2 bg-tertiary p-4 border rounded-md">
      ...
      <div className="flex items-center gap-4 text-sm">
        <AccentLink
          href={`/admin/qbank/step-one/${question.id}`}
...
function StepTwoRow({ question }: { question: StepTwoNBMEQuestion }) {
  const [isExpanded, setIsExpanded] = useState(false);
  ...
  return (
    <li className="space-y-2 bg-tertiary p-4 border rounded-md">
      ...
      <div className="flex items-center gap-4 text-sm">
        <AccentLink
          href={`/admin/qbank/step-two/${question.id}`}
          className="flex items-center gap-2"
        >
          <span>Edit</span>
          <Edit size={12} />
