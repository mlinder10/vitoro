...
import { ArrowLeft, ArrowUp, BotIcon, Loader, ChevronDown, ChevronRight } from "lucide-react";
import { RefObject, useEffect, useRef, useState } from "react";
import ReactMarkdown from "react-markdown";
import remarkGfm from 'remark-gfm';
import { promptChatWithTask, promptGeneralChat } from "@/app/(protected)/_components/test-chat";
import { Button } from "@/components/ui/button";

// Simple expandable section component matching question-card style
const ExpandableSection = ({ title, children, defaultExpanded = false, delay = 0 }: {
  title: string;
  ...
  return (
    <div className={`transition-all duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
      <button
        ...
        className="w-full flex items-center justify-between p-3 bg-secondary hover:bg-secondary/80 rounded-md mb-2 transition-colors duration-200 text-left"
      >
...
const MessageComponent = ({ content, isUser, isTyping = false }: {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`flex max-w-4xl ${isUser ? 'flex-row-reverse' : 'flex-row'} items-start gap-2`}>
        ...
        <div className={`rounded-md px-4 py-3 max-w-[600px] ${
          isUser 
            ? 'bg-custom-accent text-white' 
            : 'bg-secondary'
        }`}>
...
type PlaygroundChatCardProps = {
  question: NBMEQuestion;
  choice: QuestionChoice;
};

export default function PlaygroundChatCard({ 
  question, 
  choice 
}: PlaygroundChatCardProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [messages, setMessages] = useState<{id: number; content: React.ReactNode; isUser: boolean; rawText?: string}[]>([]);
    ...
    setTimeout(async () => {
      try {
        ...
        const serverMessages = messages.map(msg => ({
          ...
          content: msg.rawText || (typeof msg.content === 'string' ? msg.content : '')
        }));
        const res = await promptChatWithTask(task, question, choice, serverMessages);
  ...
  async function promptGeneral() {
    ...
    setTimeout(async () => {
      try {
        ...
        const res = await promptGeneralChat(question, choice, [...serverMessages, { role: "user" as const, content: inputValue }]);
...
type MessagesContainerProps = {
  messages: {id: number; content: React.ReactNode; isUser: boolean; rawText?: string}[];
...
function MessagesContainer({
  ...
  return (
    <div className="flex-1 px-4 py-4 overflow-y-auto">
      ...
      {isLoading && <MessageComponent content="" isUser={false} isTyping={true} />}
      <div ref={endRef} />
    </div>
  );
}
