...
CREATE TABLE `answered_foundational_questions` (
	...
	`is_complete` TEXT DEFAULT false NOT NULL,
	FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`foundational_question_id`) REFERENCES `foundational_questions`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE INDEX `foundational_answer_user_idx` ON `answered_foundational_questions` (`user_id`);--> statement-breakpoint
CREATE INDEX `foundational_answer_question_idx` ON `answered_foundational_questions` (`foundational_question_id`);--> statement-breakpoint
